...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query("SELECT m FROM ChatMessage m WHERE m.projectId = :projectId AND m.parentMessageId IS NULL AND ((m.sender = :user AND m.recipient = :other) OR (m.sender = :other AND m.recipient = :user)) ORDER BY m.id ASC")
    List<ChatMessage> findConversation(@Param("projectId") Long projectId, @Param("user") String user, @Param("other") String other);

    @Query("SELECT m FROM ChatMessage m WHERE m.projectId = :projectId AND m.parentMessageId IS NULL AND ((m.sender = :user AND m.recipient = :other) OR (m.sender = :other AND m.recipient = :user)) ORDER BY m.id DESC")
    ...
    @Query("SELECT m FROM ChatMessage m WHERE m.projectId = :projectId AND m.parentMessageId IS NULL AND ((LOWER(m.sender) IN :userAliases AND LOWER(m.recipient) IN :otherAliases) OR (LOWER(m.sender) IN :otherAliases AND LOWER(m.recipient) IN :userAliases)) ORDER BY m.id ASC")
    ...
    @Query("SELECT m FROM ChatMessage m WHERE m.projectId = :projectId AND m.parentMessageId IS NULL AND ((LOWER(m.sender) IN :userAliases AND LOWER(m.recipient) IN :otherAliases) OR (LOWER(m.sender) IN :otherAliases AND LOWER(m.recipient) IN :userAliases)) ORDER BY m.id DESC")

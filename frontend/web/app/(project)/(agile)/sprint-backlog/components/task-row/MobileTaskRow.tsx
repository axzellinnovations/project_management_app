...
export default function MobileTaskRow(props: TaskRowProps) {
  ...
  return (
    <div
      ...
      style={{ borderLeftColor: state.statusBorderColor }}
    ...
    >
      <div className="flex items-center w-full min-h-[72px]">
        ...
        <div className="flex-1 min-w-0 p-4 border-r border-[#F2F4F7]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${state.priorityStyle}`}>
              {state.priorityKey}
            </span>
          </div>
          {state.renaming ? (
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') void state.commitRename();
                if (e.key === 'Escape') state.setRenaming(false);
              }}
        ...
        <div className="flex-1 flex items-center gap-3 px-3 overflow-x-auto no-scrollbar bg-[#F9FAFB]/50 [scrollbar-width:none] [-webkit-overflow-scrolling:touch]">
          ...
          <div className="flex-shrink-0 flex items-center gap-1 border-r border-[#EAECF0] pr-2" onClick={(e) => e.stopPropagation()}>
            ...
            <button type="button" onClick={state.startRename} className="flex h-11 w-11 items-center justify-center rounded-lg text-[#667085] hover:text-[#175CD3] hover:bg-[#EFF8FF] active:scale-90 transition-all">
              <Pencil size={16} />
            </button>
          ...
          {/* Status */}
          <div className="flex-shrink-0" ref={state.statusRef} onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
              onClick={() => state.openStatus()}
              className={`flex h-11 min-w-[108px] items-center justify-center gap-1.5 rounded-lg px-2 text-[11px] font-bold uppercase tracking-wider shadow-sm transition-all active:scale-95 ${state.displayStyle}`}
            >
          ...
          {/* Assignee */}
          <div className="flex-shrink-0 flex items-center relative group/assignee" ref={state.assignRef} onClick={(e) => e.stopPropagation()}>
            <button type="button" onClick={() => state.openAssign()} className="flex items-center active:scale-90 transition-transform">
          ...
          {onDueDateChange && (
            <div className="flex-shrink-0 flex items-center relative" onClick={(e) => e.stopPropagation()}>
              <button
                type="button" onClick={state.openDatePicker}
                title={`Due Date: ${formatDate(task.dueDate)}`}
                className={`text-[12px] font-bold leading-none whitespace-nowrap bg-[#F2F4F7] px-2 py-2 rounded-lg min-h-[44px] ${state.dueClass === 'overdue' ? 'text-red-600 bg-red-50' : 'text-[#475467]'}`}
              >
      ...
      {state.statusOpen && state.statusRect && typeof document !== 'undefined' && createPortal(
        <div
          ref={state.statusPortalRef}
          style={{ position: 'fixed', top: `${state.statusRect.bottom + 8}px`, left: Math.min(state.statusRect.left, window.innerWidth - 180) + 'px', width: '160px' }}
      ...
      {state.assignOpen && state.assignRect && typeof document !== 'undefined' && createPortal(
        <div
          ref={state.assignPortalRef}
          style={{ position: 'fixed', top: `${state.assignRect.bottom + 8}px`, left: Math.min(state.assignRect.left, window.innerWidth - 220) + 'px', width: 'max-content', minWidth: '200px' }}
          className="z-[9999] overflow-hidden rounded-xl border border-[#D0D5DD] bg-white shadow-xl animate-in fade-in zoom-in-95 duration-200"
        >

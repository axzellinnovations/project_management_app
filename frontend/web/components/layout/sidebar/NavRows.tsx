  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-all duration-150 text-left ${
        active ? 'bg-cu-primary/10 text-cu-primary' : 'text-cu-text-secondary hover:bg-cu-hover hover:text-cu-text-primary'
      }`}
    >
...
/* ── Section Header ── */
export function SectionHeader({ label, collapsed, expanded, onToggle }: {
  ...
  return (
    <button onClick={onToggle} className="w-full flex items-center gap-2 px-2.5 py-1.5 mb-0.5 group">
      ...
      <span
        className="text-[10.5px] font-bold text-cu-text-muted uppercase tracking-widest whitespace-nowrap overflow-hidden"
        style={{
          maxWidth: collapsed ? '0px' : '150px',
          opacity: collapsed ? 0 : 1,
          transition: 'max-width 280ms cubic-bezier(0.4,0,0.2,1), opacity 200ms',
